...
library(plotly)
library(gt)
mtt_raw <- read_excel("C:\\Users\\andre\\OneDrive - unizar.es\\Laboratorio\\UCL ILDH 2023\\Results\\MTT\\MTT4_ANDRES_PDX_037.xlsx", col_names = FALSE)
...
mtt_tidy[19:24, 2]<- 0.05
mtt_tidy[13:18, 2]<- 0.075
...
```{r,message=FALSE, warning=FALSE, echo=FALSE}
custom_colors <- c("#E6E6FA", "#D8BFD8", "#C8A2C8", "#DA70D6", "#A020F0", "#8A2BE2", "#4B0082", "#320E83", "#E6E6FA")
mtt_tidy_2<-mtt_tidy%>%
  group_by(density)%>%
...
g <- ggplot(mtt_tidy_2, aes(x = concentration_bpd, y = absorbance, fill = concentration_bpd)) +
  ...
  labs(x = "BPD Concentration (µMolar)", y = "Absorbance", title="BPD Dark MTT 5k cells/well") +
