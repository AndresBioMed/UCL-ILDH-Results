...
g <- ggplot(mtt_final, aes(x = concentration_bpd, y = mean, fill = concentration_bpd, pattern=replica)) +
  ...
  theme_minimal()+scale_fill_manual(values = custom_colors)+guides(fill = FALSE) + annotate("text", size=3, x = Inf, y = Inf, hjust = 1, vjust = 1, label = paste("IC50 for BPD Light are", round(IC50,digits = 4), "for the first replica and",round(IC50_2,digits = 4), "for the second replica.", sep=" "),fontface = 'italic')+scale_fill_manual(values = custom_colors)+guides(fill = FALSE)+scale_y_continuous(labels = scales::percent, limits=c(0,1.2))
