...
# Create the plot
g <- ggplot(mtt_final, aes(x = concentration_bpd, y = absorbance, fill = concentration_bpd, pattern=replica)) +
  ...
  labs(x = "BPD Concentration (microMolar)", y = "Absorbance", title="BPD Dark MTT 10k cells/well") +
  theme_minimal()+scale_fill_manual(values = custom_colors)+guides(fill = FALSE)

g
```

***The absorbance peak in both 0.15 and 0.2 uM BPD of the Second replica is most likely due to wrongly adding 110 uL to those wells, instead of 100 uL like the rest.***
...
g <- ggplot(mtt_final, aes(x = concentration_bpd, y = absorbance, fill = concentration_bpd, pattern=replica)) +
  ...
  theme_minimal()+scale_fill_manual(values = custom_colors)+guides(fill = FALSE) + annotate("text", size=3, x = Inf, y = Inf, hjust = 1, vjust = 1, label = paste("IC50 for BPD Light are", round(IC50,digits = 4), "for the first replica and",round(IC50_2,digits = 4), "for the second replica.", sep=" "),fontface = 'italic')
